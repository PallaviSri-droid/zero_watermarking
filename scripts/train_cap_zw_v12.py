...
from zero_watermarking.v12 import CAPZWHashNet, PairAttackDataset, V12Config, train_cap_zw_v12
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train the CAP-ZW-v12 collision-first research candidate.")
    ...
    ap.add_argument("--checkpoint", default="experiments/checkpoints/cap_zw_v12_seed42.pt")
    ap.add_argument("--history", default="experiments/results/cap_zw_v12_seed42_training_history.csv")
    args = ap.parse_args()
    ...
    history = train_cap_zw_v12(
        model, loader, config,
        checkpoint=args.checkpoint,
        history_path=args.history,
    )
    print(
        f"Training complete: {CAP_ZW_VERSION if False else 'CAP-ZW-v12'} "
        f"seed={args.seed} images={len(images)} effective_samples={len(dataset)} "
        f"bits={args.bits} epochs={len(history)} device={device}"
    )
