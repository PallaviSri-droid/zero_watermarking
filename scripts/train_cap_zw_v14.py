...
from zero_watermarking.v14 import CAP_ZW_V14_VERSION, CAPZWHashNet, PairAttackDataset, V14Config, train_cap_zw_v14
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train CAP-ZW-v14 collision-safe Pareto candidate.")
    ...
    ap.add_argument("--checkpoint", default="experiments/checkpoints/cap_zw_v14_seed42.pt")
    ap.add_argument("--history", default="experiments/results/cap_zw_v14_seed42_training_history.csv")
    args = ap.parse_args()
    ...
    config = V14Config(epochs=args.epochs, batch_size=args.batch_size, lr=args.lr, nbits=args.bits, device=device)
    model = CAPZWHashNet(nbits=args.bits)
    history = train_cap_zw_v14(model, loader, config, checkpoint=args.checkpoint, history_path=args.history)
    print(f"Training complete: {CAP_ZW_V14_VERSION} seed={args.seed} images={len(images)} effective_samples={len(dataset)} bits={args.bits} epochs={len(history)} device={device}")
