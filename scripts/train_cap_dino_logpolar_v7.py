...
from zero_watermarking.cap_dino_logpolar_v7 import CAPDinoLogPolarV7, V7Config, CAP_DINO_LP_V7_VERSION, v7_objective
...
def _finite_gradients(model: torch.nn.Module) -> bool:
    return all(p.grad is None or torch.isfinite(p.grad).all().item() for p in model.parameters() if p.requires_grad)


def main() -> int:
    ap = argparse.ArgumentParser(description="Train CAP-DINO-LogPolar v7.")
    ...
    ap.add_argument("--history", default="experiments/results/cap_dino_logpolar_v7_seed42_training_history.csv")
    args = ap.parse_args()
    ...
    cfg = V7Config(bits=args.bits, device=device, attack_views=args.attack_views)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            ...
            loss, terms = v7_objective(pair, labels_t, mem, mem_labels, cfg)
            ...
            row_terms.update({
                ...
                "kept_cap": float(pair["branch_mask"][:, 0].mean()),
                "kept_dino": float(pair["branch_mask"][:, 1].mean()),
                "kept_logpolar": float(pair["branch_mask"][:, 2].mean()),
        ...
        denom = max(batches, 1)
        row = {"epoch": float(epoch + 1), "skipped_batches": float(skipped)}
        row.update({k: v / denom for k, v in sums.items()})
        history.append(row)
        out = Path(args.checkpoint); out.parent.mkdir(parents=True, exist_ok=True)
        torch.save({"version": CAP_DINO_LP_V7_VERSION, "model": model.state_dict(), "config": cfg.__dict__, "optimizer": optimizer.state_dict(), "epoch": epoch + 1, "history": history}, out)
        hp = Path(args.history); hp.parent.mkdir(parents=True, exist_ok=True); pd.DataFrame(history).to_csv(hp, index=False)
        print(f"epoch={epoch+1} loss={row.get('loss', 0):.5f} hard_rob={row.get('robustness_hard', 0):.5f} hard_tail={row.get('robustness_hard_tail', 0):.5f} entropy={row.get('observed_entropy', 0):.4f} gates={row.get('gate_0', 0):.3f}/{row.get('gate_1', 0):.3f}/{row.get('gate_2', 0):.3f} gate_max={row.get('observed_gate_max', 0):.3f} skipped={int(skipped)}")

    print(f"Training complete: {CAP_DINO_LP_V7_VERSION} seed={args.seed} images={len(images)} effective_samples={len(dataset)} bits={args.bits} epochs={args.epochs} device={device}")
