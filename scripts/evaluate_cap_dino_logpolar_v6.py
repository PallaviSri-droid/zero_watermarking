...
from zero_watermarking.cap_dino_logpolar_v6 import CAPDinoLogPolarV6, V6Config, CAP_DINO_LP_V6_VERSION
...
def load_model(checkpoint: Path, device: str):
    ...
    cfg = V6Config(**payload.get("config", {}))
    cfg.device = device
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Evaluate CAP-DINO-LogPolar v6 on the locked benchmark.")
    ...
    ap.add_argument("--out", default="experiments/results/cap_dino_logpolar_v6_eval")
    ...
    with torch.inference_mode():
        for row in frame.itertuples(index=False):
            ...
            clean, gates = model.forward(x, hard=True), model.gate(*model.encode_branches(x))
    ...
    entropy_gate = float((-(gates.clip(1e-8) * np.log(gates.clip(1e-8))).sum(axis=1)).mean())
    dominant = np.argmax(gates, axis=1)
    summary = {
        "version": str(metadata.get("version", CAP_DINO_LP_V6_VERSION)),
        "method": "CAP-DINO-LogPolar-v6-SharedGate",
        "split": args.split,
        ...
        "gate_entropy_mean": entropy_gate,
        "gate_entropy_std": float((-(gates.clip(1e-8) * np.log(gates.clip(1e-8))).sum(axis=1)).std()),
