...
from zero_watermarking.v11 import (
    CAP_ZW_V11_VERSION,
    CAPZWHashNet,
    PairAttackDataset,
    V11Config,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train CAP-ZW-v11 with selective robustness protection.")
    ...
    parser.add_argument("--binary-collision-target", type=float, default=0.135)
    parser.add_argument("--temperature", type=float, default=0.08)
    parser.add_argument("--collision-power", type=float, default=2.0)
    parser.add_argument("--guard-target", type=float, default=0.021)
    parser.add_argument("--guard-quantile", type=float, default=0.85)
    parser.add_argument("--guard-softness", type=float, default=0.008)
    parser.add_argument("--guard-lambda", type=float, default=0.55)
    parser.add_argument("--guard-growth", type=float, default=0.10)
    parser.add_argument("--guard-max", type=float, default=2.50)
    parser.add_argument("--guard-tail-weight", type=float, default=0.70)
    parser.add_argument("--guard-mean-weight", type=float, default=0.20)
    parser.add_argument("--guard-batch-fraction", type=float, default=0.25)
    parser.add_argument("--checkpoint", default="experiments/checkpoints/cap_zw_v11.pt")
    parser.add_argument("--history", default="experiments/results/cap_zw_v11_training_history.csv")
    ...
    config = V11Config(
        ...
        margin=args.margin, robust_target=args.robust_target,
        robust_softness=args.robust_softness, robustness_quantile=args.robustness_quantile,
        attack_views=args.attack_views, memory_size=args.memory_size,
        memory_warmup=args.memory_warmup, mgda_steps=args.mgda_steps,
        topk_negatives=args.topk_negatives, diversity_target=args.diversity_target,
        tail_target=args.tail_target, binary_collision_target=args.binary_collision_target,
        uniformity_temperature=args.temperature, collision_power=args.collision_power,
        nbits=args.bits, device=device,
        ...
        robustness_guard_tail_weight=args.guard_tail_weight,
        robustness_guard_mean_weight=args.guard_mean_weight,
        robustness_guard_batch_fraction=args.guard_batch_fraction,
    )
    ...
    print(
        f"Training complete: version={CAP_ZW_V11_VERSION} epochs={len(history)} "
