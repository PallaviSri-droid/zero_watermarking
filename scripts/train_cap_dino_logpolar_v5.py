...
from zero_watermarking.cap_dino_logpolar_v5 import CAPDinoLogPolarV5, V5Config, CAP_DINO_LP_V5_VERSION, v5_objective
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train CAP-DINO-LogPolar v5.")
    ...
    ap.add_argument("--history", default="experiments/results/cap_dino_logpolar_v5_seed42_training_history.csv")
    args = ap.parse_args()
    ...
    cfg = V5Config(bits=args.bits, device=device, attack_views=args.attack_views)
    ...
    for epoch in range(args.epochs):
        ...
        totals = {k: 0.0 for k in (
            "loss", "robustness", "robustness_q", "branch_consistency", "discrimination", "tail",
            "binary_collision", "entropy", "balance", "decorrelation", "gate_consistency", "gate_usage",
            "gate_sharpness", "observed_entropy", "observed_balance", "observed_robustness", "memory_size",
            "gate_0", "gate_1", "gate_2", "gate_entropy", "attack_count",
        )}
        batches = 0
        for batch in loader:
            ...
            loss, terms = v5_objective(pair, labels_t, mem, mem_labels, cfg)
            ...
            gate = pair["clean_gates"].detach()
            for k in ("loss", "robustness", "robustness_q", "branch_consistency", "discrimination", "tail",
                      "binary_collision", "entropy", "balance", "decorrelation", "gate_consistency",
                      "gate_usage", "gate_sharpness", "observed_entropy", "observed_balance", "observed_robustness"):
                key = "loss" if k == "loss" else k
                totals[key] += float(loss.detach() if k == "loss" else terms[k].detach())
            totals["memory_size"] += float(memory_codes.shape[0])
            totals["gate_0"] += float(gate[:, 0].mean())
            totals["gate_1"] += float(gate[:, 1].mean())
            totals["gate_2"] += float(gate[:, 2].mean())
            totals["gate_entropy"] += float((-(gate.clamp_min(1e-8) * gate.clamp_min(1e-8).log()).sum(dim=1)).mean())
            totals["attack_count"] += 1.0
            batches += 1
        ...
        torch.save({"version": CAP_DINO_LP_V5_VERSION, "model": model.state_dict(), "config": cfg.__dict__,
        ...
        print(f"epoch={epoch+1} loss={row['loss']:.5f} rob={row['robustness']:.5f} tail={row['tail']:.5f} "
              f"entropy={row['observed_entropy']:.4f} gates={row['gate_0']:.3f}/{row['gate_1']:.3f}/{row['gate_2']:.3f}")

    print(f"Training complete: {CAP_DINO_LP_V5_VERSION} seed={args.seed} images={len(images)} "
