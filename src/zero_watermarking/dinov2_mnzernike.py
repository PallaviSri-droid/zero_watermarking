...
def _zernike_basis(size: int, max_order: int, device: torch.device, dtype: torch.dtype) -> Tensor:
    ...
    for n, m in _zernike_pairs(max_order):
        ...
        for s in range((n - ma) // 2 + 1):
            coeff = ((-1) ** s) * factorial(n - s) / (factorial(s) * factorial((n + ma) // 2 - s) * factorial((n - ma) // 2 - s))
            radial = radial + float(coeff) * r.pow(n - 2 * s)
        angle = m * theta
        real_terms.append(torch.where(mask, radial * torch.cos(angle), torch.zeros_like(r)).reshape(-1))
        imag_terms.append(torch.where(mask, radial * torch.sin(angle), torch.zeros_like(r)).reshape(-1))
...
class DINOv2MNZernike:
    """DINOv2 dense patches + paper-style multi-scale neighborhood Zernike moments."""
    ...
    def patch_tokens(self, x: Tensor) -> Tensor:
        ...
        if x.shape[-2:] != (self.config.input_size, self.config.input_size):
            x = F.interpolate(x, size=(self.config.input_size, self.config.input_size), mode="bicubic", align_corners=False)
    ...
    def multi_scale_neighborhood_zernike(self, tokens: Tensor) -> Tensor:
        ...
        neighborhoods = F.unfold(padded, kernel_size=3).transpose(1, 2).reshape(batch, patches, dim, 9).permute(0, 1, 3, 2).contiguous()
        neighbor_mean = torch.cat([neighborhoods[:, :, :4], neighborhoods[:, :, 5:]], dim=2).mean(dim=2)
        ...
        local = torch.stack([n[:, 0:2, 0:2], n[:, 0:2, 1:3], n[:, 1:3, 0:2], n[:, 1:3, 1:3]], dim=1)
        local = local.reshape(batch, patches * 4, 4, dim)
        local_zm = self._zm_from_windows(local, size=2).reshape(batch, patches, 4, self.zm_dim).mean(dim=2)
        return torch.cat([global_zm, local_zm], dim=-1)

    @torch.inference_mode()
    def descriptor(self, x: Tensor) -> Tensor:
        return self.multi_scale_neighborhood_zernike(self.patch_tokens(x))
...
__all__ = ["PAPER_DINOV2_ZERNIKE_VERSION", "DinoV2MNZernikeConfig", "DINOv2MNZernike"]
